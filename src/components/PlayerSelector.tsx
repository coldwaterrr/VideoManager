  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-1.5 rounded-md bg-white/10 px-2.5 py-1.5 text-xs text-white transition hover:bg-white/20"
        title="切换播放器"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 z-50 w-64 rounded-lg bg-zinc-900 border border-zinc-700 shadow-xl overflow-hidden">
            <div className="px-3 py-2 text-xs text-zinc-400 border-b border-zinc-700">选择默认播放器</div>
            {PLAYER_OPTIONS.map((opt) => (
              ...
              >
                <div className={`flex-shrink-0 ${current === opt.value ? 'text-blue-400' : 'text-zinc-500'}`}>
                  {opt.icon}
                </div>
                ...
                {current === opt.value && (
                  <svg className="size-4 text-blue-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
